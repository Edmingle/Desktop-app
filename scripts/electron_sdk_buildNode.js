...
try {
  execSync(`${platform == "win32" ? "python" : "python3"} --version`, {
    stdio: [0, 1, 2],
  });
} catch (error) {
...
if (electronPackage) {
  ...
  try {
    if (platform == "darwin") {
      ...
      execSync("cp -Rf ./build/Release/zoomsdk.node ./sdk/mac", {
        stdio: [0, 1, 2],
      });
      execSync("cp -Rf ./build/Release/zoomsdk_render.node ./sdk/mac", {
        stdio: [0, 1, 2],
      });
      execSync("cp -Rf ./build/Release/zoomsdk_napi_util.node ./sdk/mac", {
        stdio: [0, 1, 2],
      });
    } else {
      execSync(
        `node-gyp rebuild --target=${electronVersion} --arch=${arch} --dist-url=https://electronjs.org/headers --release --msvs_version=2019`,
        { stdio: [0, 1, 2] },
      );
    }
  } catch (error) {
