...
void ZMeetingConfigWrap::onWebinarNeedInputScreenName()
{
    if (m_pSink) {
        m_pSink->onWebinarNeedInputScreenName();
    }
}

ZNSDKError ZMeetingConfigWrap::InputScreenName(ZoomSTRING zn_name)
{
    ZoomSDKWebinarInputScreenNameHandler *handler = [[meetingServiceDelegate share] getWebinarInputScreenNameHandler];
    if (handler) {
        NSString *name = [NSString stringWithUTF8String:zn_name.c_str()];
        ZoomSDKError ret = [handler inputName:name];
...
ZNSDKError ZMeetingConfigWrap::CancelJoinWebinar()
{
    ZoomSDKWebinarInputScreenNameHandler *handler = [[meetingServiceDelegate share] getWebinarInputScreenNameHandler];
    if (!handler) {
