...
{
	ZoomSTRING ret = L"";
	const wchar_t* znPrompt = ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().GetMeetingWebinarController().getWebinarLegalNoticesPrompt();
	if (znPrompt)
	{
		ret = znPrompt;
	}
...
ZNWebinarLegalNoticesExplainedInfo ZAuthServiceWrap::GetWebinalLegalNoticesExplained()
{
	...
	bool bret = ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().GetMeetingWebinarController().getWebinarLegalNoticesExplained(sdk_explainedInfo);
	if (bret)
	{
