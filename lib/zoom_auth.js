import { ZoomAuthResult, ZoomSDKError } from "./settings";
import { ZoomDirectShareHelper } from "./zoom_direct_share_helper";
import pb from "./electron_sdk_pb";

export const ZoomAuth = (function () {
  let instance;
  ...
  function init(opts) {
    ...
    let _onZoomAuthIdentityExpired =
      clientOpts.onZoomAuthIdentityExpired || null;
    ...
    return {
      ...
      SetOnZoomIdentityExpiredCB: function (onZoomIdentityExpired) {
        if (
          ...
          onZoomIdentityExpired instanceof Function
        ) {
      ...
      SetOnZoomAuthIdentityExpiredCB: function (onZoomAuthIdentityExpired) {
        if (
          ...
          onZoomAuthIdentityExpired instanceof Function
        ) {
      ...
      SetLoginReturnWithReasonCB: function (onLoginReturnWithReason) {
        if (
          ...
          onLoginReturnWithReason instanceof Function
        ) {
      ...
      GenerateSSOLoginWebURL: function (opts) {
        if (_addon) {
          ...
          try {
            let GenerateSSOLoginWebURLParams =
              new pb.GenerateSSOLoginWebURLParams();
            GenerateSSOLoginWebURLParams.setPrefixofvanityurl(
              prefixOfVanityUrl
            );
        ...
        return _isSDKAuthentication
          ? ZoomSDKError.SDKERR_UNAUTHENTICATION
          : ZoomSDKError.SDKERR_UNINITIALIZE;
      },
      /**
      ...
      SSOLoginWithWebUriProtocol: function (opts) {
        if (_addon) {
          ...
          try {
            let SSOLoginWithWebUriProtocolParams =
              new pb.SSOLoginWithWebUriProtocolParams();
       ...
       */
      Logout: function () {
        if (_isSDKAuthentication && _addon) return _addon.Logout();
        return _isSDKAuthentication
          ? ZoomSDKError.SDKERR_UNAUTHENTICATION
          : ZoomSDKError.SDKERR_UNINITIALIZE;
      },
      ...
      AuthWithJwtToken: function (sdk_context) {
        if (_addon) {
          try {
            let AuthWithJwtTokenParams = new pb.AuthWithJwtTokenParams();
      ...
      GetDirectShare: function (opts) {
        if (_addon) {
          ...
          return ZoomDirectShareHelper.getInstance(clientOpts);
        }
        return _isSDKAuthentication
          ? ZoomSDKError.SDKERR_UNAUTHENTICATION
          : ZoomSDKError.SDKERR_UNINITIALIZE;
