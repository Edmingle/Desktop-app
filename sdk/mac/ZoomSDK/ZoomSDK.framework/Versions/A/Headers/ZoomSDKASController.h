...
@interface ZoomSDKShareSource :NSObject
{
    ...
    //Avaliable only for Zoom UI mode.
 ...
 * @param userID The ID of user who is sharing.
 * @return If the function succeeds, it will return ZoomSDKError_Succuss, otherwise not.
 * @note For custom style mode, this interface is only valid after subscribing the sharing content from the specified user by calling "-(ZoomSDKError)createShareElement:(ZoomSDKShareElement**)element" successfully and set the share element userid.
 */
 ...
 * @param reason The reason that no one can start sharing, see {@link ZoomSDKCannotShareReasonType}.
